...
//!
//! The structure of this crate and its trait definitions are based on `serde` but not compatible
//! with it (primarily because `serde` doesn't support `async`). Many trait methods are copied
//! directly from `serde` with minimal modification.
//!
